            ...
            : sorted.map(t => {
              ...
              return (
                <tr key={t.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-3 py-2.5">{collabs.length === 0 ? <span className="text-gray-300 text-xs">—</span> : <div className="flex flex-wrap gap-1">{collabs.map(col => col.user ? <PersonChip key={col.id} name={col.user.full_name} assignedHours={col.assigned_hours} usedHours={t.hours_logged}/> : null)}</div>}</td>
