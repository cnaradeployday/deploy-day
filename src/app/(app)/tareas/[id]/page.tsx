...
export default async function TareaDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string>> }) {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className={`grid gap-3 mb-4 ${canVerHorasEstimadas ? 'grid-cols-2 sm:grid-cols-4' : 'grid-cols-2 sm:grid-cols-3'}`}>
        {[
          canVerHorasEstimadas ? { label: 'Horas estimadas', value: t.estimated_hours ? t.estimated_hours + 'h' : '—' } : null,
        ...
        ].filter(Boolean).map(({ label, value, alert }: any) => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        ...
        <TimeEntriesList
          ...
          canEdit={isAdmin || canCargarHorasOtros}
