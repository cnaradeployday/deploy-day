...
export default async function PresentacionesClientePage({ params }: { params: Promise<{ clientId: string }> }) {
  ...
  const [{ data: cliente }, { data: presentaciones }, { data: favoritosPres }, { data: favoritosClientes }, { data: shares }, puedeEscribir] = await Promise.all([
  ...
  const presentacionesEnriquecidas = (presentaciones ?? []).map(p => ({
    ...
    esFavorita: favPresSet.has(p.id), tieneCompartidos: compartidasSet.has(p.id),
