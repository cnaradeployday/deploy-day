...
import { CheckCircle, Upload, Clock, DollarSign, FileText } from 'lucide-react'
...
export default function LiquidacionesColaborador({ liquidaciones, userId, profile }: {
  liquidaciones: any[]
  userId: string
  profile: any
}) {
  ...
  async function subirFactura(id: string, file: File) {
    ...
    const { error: uploadError } = await sb.storage.from('facturas').upload(path, file, { upsert: true })
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400 mt-0.5">Confirmá tus horas al cierre de cada mes y subí tu factura cuando esté aprobado</p>
      </div>
          ...
          {liquidaciones.map(liq => (
            <div key={liq.id} className={'bg-white rounded-2xl border p-5 ' + (liq.estado === 'aprobado_gerente' ? 'border-amber-200' : liq.estado === 'pagado' ? 'border-green-200' : 'border-gray-100')}>
              <div className="flex items-start justify-between mb-4">
                <div>
                  <p className="font-semibold text-gray-900">{liq.mes.slice(0,4)} · {new Date(liq.mes + '-01').toLocaleString('es-AR', { month: 'long' })}</p>
                ...
                <span className={'text-xs px-2.5 py-1 rounded-full ' + estadoColors[liq.estado]}>
                  {estadoLabels[liq.estado]}
                </span>
              </div>

              {liq.notas_gerente && (
                <div className="bg-amber-50 rounded-xl px-4 py-2.5 mb-4 text-sm text-amber-700">
                  <span className="font-medium">Nota del gerente: </span>{liq.notas_gerente}
                </div>
              )}

              {liq.estado === 'borrador' && (
                <button onClick={() => confirmar(liq.id)} disabled={loading === liq.id}
                  className="w-full flex items-center justify-center gap-2 py-2.5 bg-[#1B9BF0] hover:bg-[#0F7ACC] text-white rounded-xl text-sm font-semibold transition-all disabled:opacity-50">
                  <CheckCircle size={15}/> Confirmar horas de este mes
                </button>
              )}
              ...
              {liq.estado === 'aprobado_gerente' && (
                <div>
                  <p className="text-sm text-amber-700 font-medium mb-3">
                    ¡Aprobado! Subí tu factura por ${Number(liq.monto_total).toLocaleString()}
                  </p>
              ...
              {liq.estado === 'factura_subida' && (
                <div className="flex items-center gap-2 text-sm text-purple-600">
                  <FileText size={15}/> Factura enviada: {liq.factura_nombre} — esperando pago
                </div>
              )}

              {liq.estado === 'pagado' && (
                <div className="flex items-center gap-2 text-sm text-green-600 font-medium">
                  <CheckCircle size={15}/> Pagado el {liq.pagado_at ? new Date(liq.pagado_at).toLocaleDateString('es-AR') : '—'}
                </div>
              )}
            </div>
          ))}
