      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        {!messages.length && (
          <div className="flex flex-col items-center justify-center h-full text-gray-400">
            {isGlobal ? <Hash size={28} className="mb-2 opacity-20"/> : <Lock size={28} className="mb-2 opacity-20"/>}
        ...
        <div className="mx-4 mb-2 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
          {suggestions.map((s, i) => (
            <button key={s.id} onClick={() => applySuggestion(s)}
              ...
              <span className={'w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ' + (s.type === 'user' ? 'bg-purple-100 text-purple-600' : s.type === 'project' ? 'bg-green-100 text-green-600' : 'bg-amber-100 text-amber-600')}>
