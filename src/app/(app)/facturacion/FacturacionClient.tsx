...
function nombreMes(m: string) { return new Date(m + '-15').toLocaleString('es-AR', { month: 'long', year: 'numeric' }) }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-semibold text-gray-900 flex items-center gap-2"><Receipt size={18} className="text-[#1B9BF0]"/> Facturación</h1>
          <p className="text-sm text-gray-400 mt-0.5 capitalize">{nombreMes(mes)} · {filas.length} clientes{tipoCambio && fechaCotiz && <span className="ml-2">· TC: <strong>${Number(tipoCambio).toLocaleString('es-AR')}</strong> <span className="text-gray-300">({new Date(fechaCotiz + 'T12:00:00').toLocaleDateString('es-AR')})</span></span>}</p>
        </div>
      ...
      {!filas.length ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="grid grid-cols-12 px-5 py-3 text-xs font-medium text-gray-400 bg-gray-50 border-b border-gray-100">
            <span className="col-span-2">Cliente / Proyecto</span>
            <span className="col-span-1 text-right">Vendidas</span>
            <span className="col-span-1 text-right">Ejec.</span>
