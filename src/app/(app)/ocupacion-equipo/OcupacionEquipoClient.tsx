...
import { ChevronUp, ChevronDown, ChevronRight, Download, RefreshCw, Users } from 'lucide-react'
        ...
        <table className="w-full">
          ...
          <tbody>
            {sorted.map(f => {
              ...
              const hasDesglose = f.desglose?.length > 0

              return (
                <>
                  <tr key={f.id} className={'border-b border-gray-50 transition-colors ' + (isExpanded ? 'bg-gray-50' : 'hover:bg-gray-50')}>
                    <td className="px-4 py-3">
                      <button
                        onClick={() => hasDesglose && toggleExpand(f.id)}
                        className={'flex items-center gap-1.5 text-sm font-medium text-gray-900 ' + (hasDesglose ? 'hover:text-[#1B9BF0] cursor-pointer' : 'cursor-default')}
                      >
                        <ChevronRight size={14} className={'text-gray-300 transition-transform shrink-0 ' + (isExpanded ? 'rotate-90' : '')}
                          style={{ opacity: hasDesglose ? 1 : 0 }}
                        />
                        {f.nombre}
                      </button>
                    </td>
                  ...
                  {isExpanded && (f.desglose ?? []).map((d: any) => (
                    <tr key={f.id + '-' + d.nombre} className="border-b border-gray-50 last:border-0 bg-blue-50/30">
                      <td className="pl-10 pr-4 py-2">
                        <span className="text-xs text-gray-500 font-medium">{d.nombre}</span>
                      </td>
                      <td className="px-4 py-2 text-xs text-gray-300">—</td>
                      <td className="px-4 py-2 text-xs font-medium text-gray-600">
                        {d.estimadas > 0 ? d.estimadas + 'h' : <span className="text-gray-300">—</span>}
                      </td>
                      <td className="px-4 py-2 text-xs text-gray-300">—</td>
                      <td className="px-4 py-2 text-xs font-medium text-[#1B9BF0]">
                        {d.realizadas > 0 ? d.realizadas + 'h' : <span className="text-gray-300">—</span>}
                      </td>
                    </tr>
                  ))}
                </>
